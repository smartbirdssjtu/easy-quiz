...
const Home: React.FC = () => {
  ...
  const [initialData, setInitialData] = useState<string[][]>([]); // New state to store the initial data
  ...
  const [savedLimit, setSavedLimit] = useState<number>(limit); // Store the limit when starting the test
  const [savedEntryLimit, setSavedEntryLimit] = useState<number>(entryLimit); // Store the entryLimit when starting the test
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      // Store the uploaded data in initialData
  ...
  const initializeQuiz = (
    ...
    const limitedUniqueValues = shuffleArray(Array.from(uniqueSet)).slice(
      0,
      limit
    ); // Apply the limit
  ...
  const handleAnotherQuiz = () => {
    // Use initialData to reset the quiz
    setSavedLimit(limit);
    setSavedEntryLimit(entryLimit);
    initializeQuiz(initialData, limit, entryLimit);
  };

  const handleSubmit = () => {
    if (randomKey && availableKeys.length > 0) {
      const selectedValue = (
        document.getElementById("dropdown") as HTMLSelectElement
      ).value;
  ...
  const handleStartTest = () => {
    ...
    // Initialize the quiz with the current settings
    initializeQuiz(initialData, limit, entryLimit);
  };
        ...
        <>
          <div style={{ marginTop: "20px" }}>
            ...
            >
              ...
              <button onClick={handleEnd} style={{ marginLeft: "20px" }}>
                End
              </button>
            </div>
          </div>
